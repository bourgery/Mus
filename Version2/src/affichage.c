...
void afficher_toutes_les_mains(SDL_Surface *ecran, Partie *partie, SDL_Rect tab[]) {
  ...
  for (int i = 0; i < 4; i++)
    graphique_affiche_main(ecran, partie->joueurs[i].main, tab, i);
}
...
void jeter_carte(SDL_Surface *ecran, Partie *partie, SDL_Rect tab[], Direction d, bool choix_carte[]) {
  ...
  if(d == BAS || d == HAUT){
    image = IMG_Load("ressource/jeu_complet.png");
    pos.x = LARGEUR_CARTE * 12;
    pos.y = 0;
    pos.w = LARGEUR_CARTE;
    pos.h = LONGUEUR_CARTE;
    ...
    image = IMG_Load("ressource/jeu_complet_tourner_droite.png");
    pos.x = 0;
    pos.y = 0;
    pos.w = LONGUEUR_CARTE;
    pos.h = LARGEUR_CARTE;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (choix_carte[i]) {
      if (d == BAS) {
        ...
        pos_carte.y = ecran->h - (LONGUEUR_CARTE + LONGUEUR_CARTE / 4);
        ...
        pos_carte.x = ecran->w / 2 - (-2 * LARGEUR_CARTE + ((4 - i) * LARGEUR_CARTE));
        pos_carte.y = LONGUEUR_CARTE / 4;
        animation_distribuer_carte(ecran, image, pos, pos_carte);
        while (pos_carte.y < (5 * LONGUEUR_CARTE / 4)) {
          animation_distribuer_carte(ecran, image, pos, pos_carte);
          pos_carte.y += NB_PIXEL_DECALAGE;
        }
        while (abs(pos_carte.x - pos_final.x) != 0 || pos_carte.y < pos_final.y) {
        ...
        affiche_dos(ecran, HAUT, pos_carte);
      }
      else if (d == DROITE) {
        pos_carte.x = ecran->w - (LONGUEUR_CARTE + LONGUEUR_CARTE / 4);
        pos_carte.y = ecran->h / 2 - (-2 * LARGEUR_CARTE + ((4 - i) * LARGEUR_CARTE));
        animation_distribuer_carte(ecran, image, pos, pos_carte);
        while (pos_carte.x > ecran->w - (5 * LONGUEUR_CARTE / 2)) {
          animation_distribuer_carte(ecran, image, pos, pos_carte);
          pos_carte.x -= NB_PIXEL_DECALAGE;
        }
        while (abs(pos_carte.y - pos_final.y) != 0 || pos_carte.x > pos_final.x) {
          animation_distribuer_carte(ecran, image, pos, pos_carte);
          if (pos_carte.x > pos_final.x)
            pos_carte.x -= NB_PIXEL_DECALAGE;
          if (pos_carte.y < pos_final.y)
        ...
        affiche_dos(ecran, DROITE, pos_carte);
      }
      else{
        pos_carte.x = LONGUEUR_CARTE / 4;
        pos_carte.y = ecran->h / 2 - (-2 * LARGEUR_CARTE + ((4 - i) * LARGEUR_CARTE));
        animation_distribuer_carte(ecran, image, pos, pos_carte);
        while (pos_carte.x < (5 * LONGUEUR_CARTE / 4)) {
          animation_distribuer_carte(ecran, image, pos, pos_carte);
          pos_carte.x += NB_PIXEL_DECALAGE;
        }
        while (abs(pos_carte.y - pos_final.y) != 0 || pos_carte.x < pos_final.x) {
        ...
        affiche_dos(ecran, GAUCHE, pos_carte);
      }
    }
  }
...
void enlever_carte_jetee(SDL_Surface *ecran) {
  SDL_Surface *fond = SDL_CreateRGBSurface(SDL_HWSURFACE, LONGUEUR_CARTE + 20, LONGUEUR_CARTE + 20, 32, 0, 0, 0, 0);
  SDL_FillRect(fond, NULL, COULEUR_FOND);
  SDL_Rect pos;
  pos.x = ecran->w / 2;
  pos.y = ecran->h / 2 - 5;
...
void distribuer_carte_manquante(SDL_Surface *ecran, bool choix_carte[], Direction d, int esku) {
  ...
  if (d == BAS || d == HAUT) {
    ...
    fond_vert = SDL_CreateRGBSurface(SDL_HWSURFACE, LONGUEUR_CARTE, 4 * LARGEUR_CARTE, 32, 0, 0, 0, 0);
    pos_fond_vert.y = ecran->h / 2 - (-2 * LARGEUR_CARTE + (4 * LARGEUR_CARTE));
    if(d == DROITE)
      pos_fond_vert.x = ecran->w - (LONGUEUR_CARTE + LONGUEUR_CARTE / 4);
    else
      pos_fond_vert.x = LONGUEUR_CARTE / 4;
  }
  SDL_FillRect(fond_vert, NULL, COULEUR_FOND);
  ...
  for (int i = 0; i < nb; i++) {
    if (d == BAS || d == HAUT)
      ...
      pos_dos.y = ecran->h / 2 - (-2 * LARGEUR_CARTE + ((i + 1) * LARGEUR_CARTE));
...
void texte_correspondant_mise(char txt[], int mise, int mise_precedente) {
  char m[2];
  switch (mise) {
  case 0:
    strcpy(txt, "paso");
  ...
  default:
    itoa(mise, m);
    if (mise == 2 && mise_precedente == 0) {
      strcpy(txt, "imido");
      break;
    }
    if (mise_precedente == 0) {
      strcpy(txt, "misa ");
      txt[5] = m[0];
      txt[6] = m[1];
      txt[7] = m[2];
...
void afficher_message_mise(SDL_Surface *ecran, Partie *partie, int mise, int joueur, int mise_precedente) {
  SDL_Surface *fond = NULL, *texte = NULL;
  SDL_Color couleur_blanche = {255, 255, 255};
  TTF_Font *police = TTF_OpenFont(CHOIX_POLICE(20));
  SDL_Rect pos;
  char txt[20];
  texte_correspondant_mise(txt, mise, mise_precedente);
  texte = TTF_RenderText_Blended(police, txt, couleur_blanche);
  int place = joueur;
  if (place == BAS || place == HAUT) {
    pos.x = ecran->w / 2 - texte ->w / 2;
    if (place == BAS)
    ...
    if (place == DROITE)
      pos.x = ecran->w - 2 * LONGUEUR_CARTE;
    else
...
void afficher_message_mus(SDL_Surface *ecran, Direction d, bool reponse) {
  SDL_Surface *texte = NULL, *fond = NULL;
  SDL_Color couleur_blanche = {255, 255, 255};
  TTF_Font *police = TTF_OpenFont(CHOIX_POLICE(20));
  SDL_Rect pos;
  if (reponse)
    texte = TTF_RenderText_Blended(police, "Mus", couleur_blanche);
  else
    texte = TTF_RenderText_Blended(police, "Mintza", couleur_blanche);
  if (d == BAS || d == HAUT) {
    pos.x = ecran->w / 2 - texte->w / 2;
    if (d == BAS)
      pos.y = ecran->h - 1.5 * LONGUEUR_CARTE;
...
void message_oui_non(SDL_Surface *ecran, Partie *partie, bool tab_joueur[]) {
  ...
  affiche_paquet_esku(ecran, esku);
  for (int i = 0; i < 4; i++) {
    affiche_message_oui_non(ecran, (esku + i) % 4, tab_joueur[(esku + i) % 4]);
    SDL_Delay(500);
  }
  for (int i = 0; i < 4; i++)
