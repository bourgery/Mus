...
void init_joueur(SDL_Surface *ecran, Partie *partie) {
  ...
  for (int i = 0; i < 4; i++) {
    partie->joueurs[i].num_joueur = i;
    strcpy(partie->joueurs[i].nom, nom[i]);
    partie->joueurs[i].IA = true;
  }
  partie->joueurs[0].IA = false;
}
...
bool choix_mus_equipe(SDL_Surface *ecran, int equipe, Partie *partie, SDL_Rect tab[]) {
  bool choix_j1, choix_j2;
  int esku = partie->nb_tours % 4;
  equipe = (esku == 0 || esku == 1) ? equipe : equipe + 2;
  if (partie->joueurs[equipe].IA) {
    choix_j1 = choix_mus_IA(partie->joueurs[equipe], partie->paquet.cartes_jetees);
    afficher_message_mus(ecran, equipe, choix_j1);
  }
  else {
    graphique_affiche_main(ecran, partie->joueurs[equipe].main, tab, 0);
    choix_j1 = graphique_choix_mus_joueur(ecran);
  }
  if (!choix_j1)
    return false;
  if (partie->joueurs[(equipe + 2) % 4].IA) {
    choix_j2 = choix_mus_IA(partie->joueurs[(equipe + 2) % 4], partie->paquet.cartes_jetees);
    afficher_message_mus(ecran, (equipe + 2) % 4, choix_j2);
  }
  else {
    graphique_affiche_main(ecran, partie->joueurs[(equipe + 2) % 4].main, tab, 0);
    choix_j2 = graphique_choix_mus_joueur(ecran);
  }
  return choix_j1 && choix_j2;
}

bool est_mus(SDL_Surface *ecran, SDL_Rect tab[], Partie *partie) {
  int esku = partie->nb_tours % 4;
  if (esku == 0 || esku == 2)
    return (choix_mus_equipe(ecran, 0, partie, tab) && choix_mus_equipe(ecran, 1, partie, tab));
  else
    return (choix_mus_equipe(ecran, 1, partie, tab) && choix_mus_equipe(ecran, 0, partie, tab));
}
...
void choix_carte_mus(SDL_Surface *ecran, Partie *partie, SDL_Rect tab[], int joueur, bool cartes[]) {
  if (partie->joueurs[joueur].IA)
    choix_carte_mus_IA(partie->joueurs[joueur], cartes);
  else
    graphique_choix_carte_mus(ecran, partie, tab, joueur, cartes);
}
...
void distribution_mus(SDL_Surface *ecran, Partie *partie, SDL_Rect tab[]) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    affiche_nom_joueurs(ecran, partie, 0);
    choix_carte_mus(ecran, partie, tab, (esku + i) % 4, cartes); //renvoie un tab de bool (true si la carte est jetee)
    jeter_carte(ecran, partie, tab, (esku + i) % 4, cartes);
    distribuer_carte_manquante(ecran, cartes, (esku + i) % 4, esku);
...
int phase_de_mise(SDL_Surface *ecran, SDL_Rect tab[], Partie *partie, Phase *phase, int mise_precedente, int joueur, int mise_totale, int mise_en_jeu) {
  int m;
  if (partie->joueurs[joueur].IA) {
    m = phase_de_mise_IA(partie, phase, mise_precedente, mise_totale, joueur, mise_en_jeu);
    afficher_message_mise(ecran, partie, m, joueur, mise_precedente);
    return m;
  }
  else {
    m = graphique_phase_de_mise(ecran, tab, partie, mise_precedente, joueur);
    afficher_message_mise(ecran, partie, m, joueur, mise_precedente);
    return m;
  }
}
...
int jouer(SDL_Surface *ecran, SDL_Rect tab[], Partie *partie, Phase *phase, bool tab_joueur[]) {
  ...
  while (joueur_dispo && !est_fini_tour(mise_en_cours) && !(mise_en_cours[1] == 0 && miseur == partie->nb_tours % 4 && compteur != 0)) {
    if (tab_joueur[miseur] == 1) {
      mise = phase_de_mise(ecran, tab, partie, phase, mise, miseur, mise_en_cours[0] + mise_en_cours[1], mise_en_cours[0]);
      if (mise > 1) {         //le joueur surenchérit
        mise_en_cours[0] += mise_en_cours[1];
        mise_en_cours[1] = mise;
        phase->mise = mise_en_cours[0] + mise;
        ...
        if (mise == -3) {     //le joueur fait idoki
          if (tab_joueur[ (miseur + 2) % 4 ] == 1) {
            mise = phase_de_mise(ecran, tab, partie, phase, mise, (miseur + 2) % 4, mise_en_cours[0] + mise_en_cours[1], mise_en_cours[0]); //on demande à son collègue
          ...
          if (mise == 1) {   //le joueur veut tira
            if (tab_joueur[ (miseur + 2) % 4 ] == 1) {
              mise = phase_de_mise(ecran, tab, partie, phase, mise_en_cours[1], (miseur + 2) % 4, mise_en_cours[0] + mise_en_cours[1], mise_en_cours[0]);  //on demande à son collègue
...
void jeu(SDL_Surface * ecran) {
  ...
  while (!est_finie_partie(partie)) {
    ...
    while (!est_finie_manche(partie)) {
      ...
      graphique_affiche_main(ecran, partie->joueurs[0].main, carte_graphique, 0);
